...
# MGS Convert Xsec Traditional to Stacked
# Created by Sarah Francis, Minnesota Geological Survey
# Created Date: September 2022, updated October 2022, November 2023
'''
This script takes any feature class that was created in the traditional
cross section systems and converts the feature class to the stacked coordinate 
system. Vertical exaggeration is created by stretching the y axis, and cross
setcions are spaced vertically based on mn_et_id.
'''
#%% 
...
if run_location == "Pro":
    ...
    in_fc_etid_field = arcpy.GetParameterAsText(1) #input feature class cross section ID field
    out_fc = arcpy.GetParameterAsText(2) #output feature class
    xsln_fc = arcpy.GetParameterAsText(3) #mapview cross section lines
    xsln_etid_field = arcpy.GetParameterAsText(4) #cross section line id field
    in_vertical_exaggeration = int(arcpy.GetParameterAsText(5)) #input vertical exaggeration
...
if shape == "Point":
    printit("Converting point data to stacked coordinates.")
    ...
    with arcpy.da.SearchCursor(xsln_fc, xsln_cursor_fields) as xsln_cursor:
        for line in xsln_cursor:
            ...
            #define minimum (westernmost) x UTM coordinate that will be added to original x
...
# 8 Convert line data
if shape == "Polyline":
    printit("Converting polyline data to stacked coordinates.")
...
if shape == "Polygon":
    printit("Converting polygon data to stacked coordinates.")
    #define fields for search cursor, adding mn_et_id if original files are in the stacked system
    xsln_cursor_fields = ['SHAPE@', xsln_etid_field, 'mn_et_id']
    ...
    with arcpy.da.SearchCursor(xsln_fc, xsln_cursor_fields) as xsln_cursor:
        for xsln_line in xsln_cursor:
            ...
            #search through strat vertex points along current xsln
            with arcpy.da.UpdateCursor(in_fc, ['SHAPE@', unique_id_field], where_clause) as poly_cursor:
